...
namespace MAP
{
    public class Patcher
    {
        ...
        */
        ...
        /// <summary>
        /// Parses a byte[] to a list of patches.
        /// </summary>
        static public List<Patch> ParseByteArrayToPatches(byte[] patchBytes)
        {
            return Serialization_Functions.TypeFromByteArray<List<Patch>>(patchBytes);
        }


        /// <summary>
        /// Encodes a list of patches to a byte[] for export.
        /// </summary>
        static public byte[] EncodePatchesToByteArray(List<Patch> patches)
        {
            return Serialization_Functions.ObjectToByteArray(patches);
        }
        ...
        static public PatchResults ApplyPatches(string filePath, List<Patch> patches)
        {
            ...
            for(int currentPatch = 0; currentPatch < patches.Count; currentPatch++)
            {
                ...
                {
                    // if we're replacing text with the same length, use a more efficent replacement method
                    if
                    (
                    patches[currentPatch + 1].operation == Operation.INSERT &&
                    patches[currentPatch].startingIndex == patches[currentPatch + 1].startingIndex &&
                    patches[currentPatch].length == patches[currentPatch + 1].length
                    )
                    {
                        for (int i = patches[currentPatch].startingIndex; i < patches[currentPatch].length; i++)
                        {
                            tempContent[i] = patches[currentPatch + 1].content[i];
                            currentPatch++;
                        }
                    }
                    else
